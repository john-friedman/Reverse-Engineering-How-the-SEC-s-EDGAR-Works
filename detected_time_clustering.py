...
# Extract dates and filter where detected date == acceptance date
df = df.with_columns([
...
# Round detectedTime to 5-minute intervals and extract time only
same_day = same_day.with_columns([
    (pl.col("detectedTime").dt.truncate("5m")).alias("interval_5m"),
    (pl.col("detectedTime").dt.truncate("5m").dt.time()).alias("time_only")
])

# Count filings per date-time interval (to calculate daily totals)
daily_time_counts = same_day.group_by(["detected_date", "time_only"]).agg(
    pl.count().alias("filing_count")
)

# Calculate total filings per day
daily_totals = same_day.group_by("detected_date").agg(
    pl.count().alias("daily_total")
)

# Join to get daily totals for each interval
daily_time_counts = daily_time_counts.join(daily_totals, on="detected_date")
...
ax1.set_ylabel("Percent of Daily Filings Detected (%)")
...
ax2.set_ylabel("Cumulative Percent of Daily Filings Detected (%)")
...
plt.tight_layout()
plt.savefig("plots/filings_percent_and_cumsum_per_5m_interval_by_time.png", dpi=300)
plt.show()
...
ax3.set_ylabel("Percent of Daily Filings Detected (%) [Log Scale]")
...
ax4.set_ylabel("Cumulative Percent of Daily Filings Detected (%)")
...
plt.tight_layout()
plt.savefig("plots/filings_percent_and_cumsum_per_5m_interval_by_time_log.png", dpi=300)
plt.show()